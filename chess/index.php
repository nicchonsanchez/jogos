...
$accent   = '#f59e0b';
$title    = 'XADREZ';
$subtitle = 'Multiplayer em tempo real · vs Bot';
$backHref = '../';
?>
<!DOCTYPE html>
<html lang="pt-BR">
    ...
    <div class="form-box open">
      <div class="form-row">
        ...
        <div class="opt-row" id="diff-row">
          ...
          <button class="opt-btn active" data-diff="2">Médio</button>
          <button class="opt-btn" data-diff="3">Difícil</button>
        </div>
      </div>
      <div class="form-row">
        <label>Suas peças</label>
        <div class="opt-row" id="bcolor-row">
...
let selColor = 'white', selBColor = 'white', selDiff = 2;

document.querySelectorAll('#pvp-section .opt-row:first-of-type .opt-btn').forEach(btn => {
  btn.addEventListener('click', function() {
    this.closest('.opt-row').querySelectorAll('.opt-btn').forEach(b => b.classList.remove('active'));
...
document.getElementById('bcolor-row').querySelectorAll('.opt-btn').forEach(btn => {
  btn.addEventListener('click', function() {
    document.getElementById('bcolor-row').querySelectorAll('.opt-btn').forEach(b => b.classList.remove('active'));
    this.classList.add('active');
...
document.getElementById('btn-create').addEventListener('click', async () => {
  ...
  const res = await fetch('api.php', {
    ...
    body: JSON.stringify({action:'create', username:player.username, name:player.name, game:'chess', color, roomName: document.getElementById('room-name').value.trim()})
